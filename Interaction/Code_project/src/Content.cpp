...
void Content::LoadGLTF(string filename, int saver){
	...
	vaoAndEbos = BindModel(model, saver);

}


void Content::BindMesh(map<int, GLuint> &vbos, Model &model, Mesh &mesh, int saver)
{
	...
	for (size_t i = 0; i < mesh.primitives.size(); ++i)
	{
		...
		if (model.textures.size() > 0)
		{
			...
			if (tex.source > -1)
			{
				...
				glActiveTexture(GL_TEXTURE0 + saver);
...
// bind models
void Content::BindModelNodes(map<int, GLuint> &vbos, Model &model, Node &node, int saver)
{
	if ((node.mesh >= 0) && (node.mesh < model.meshes.size()))
	{
		BindMesh(vbos, model, model.meshes[node.mesh], saver);
	}

	for (size_t i = 0; i < node.children.size(); i++)
	{
		assert((node.children[i] >= 0) && (node.children[i] < model.nodes.size()));
		BindModelNodes(vbos, model, model.nodes[node.children[i]], saver);
	}
}

pair<GLuint, map<int, GLuint>> Content::BindModel(Model &model, int saver)
{
	map<int, GLuint> vbos;
	GLuint vao;
	...
	for (size_t i = 0; i < scene.nodes.size(); ++i)
	{
		...
		BindModelNodes(vbos, model, model.nodes[scene.nodes[i]], saver);
	}
